...
# 绕过 create_model，直接实例化模型，确保参数传递
#
...
# [关键] 引入 RepSS 模型类和配置
try:
    from models.fastvit_repss import FastViT, default_cfgs, partial, RepCPE
except ImportError:
    print("[ERROR] 找不到 models/fastvit_repss.py！")
    exit(1)
...
# ... (HSIDataset 类保持不变，省略以节省篇幅，请保留你原文件里的这部分) ...
...
# ... (参数定义 config_parser 和 parser 保持不变，请保留) ...
...
def main():
    ...
    # =================================================================
    # [核心修改] 直接实例化 FastViT，绕过 create_model
    # =================================================================
    _logger.info(f"Creating RepSS-Mixer model: {args.model} (Direct Init)")
    
    # 手动定义 MA36 的配置 (复制自 fastvit_repss.py)
    layers = [6, 6, 18, 6]
    ...
    if args.initial_checkpoint and os.path.exists(args.initial_checkpoint):
        ...
        print(f"[INFO] Successfully loaded {len(pretrained_dict)} layers from ImageNet weights.")
    ...
    if args.experiment:
        ...
        saver = CheckpointSaver(
            ...
            amp_scaler=loss_scaler, checkpoint_dir=output_dir, max_history=5,
