...
## Day 8: 多场景泛化性验证 (Generalization Test)

### 1. 实验动机
- **背景**: 前期实验主要基于 Indian Pines (农业场景) 进行架构调优。为了验证 **FastViT-RepSS (Small Patch)** 架构是否具有普适性，而非仅对特定数据集过拟合。
- **目标**: 在不调整任何超参数 (Fixed Hyperparameters) 的前提下，将模型迁移至 **Pavia University (城市场景)** 和 **Salinas (山谷农业)** 数据集，评估其跨传感器、跨场景的鲁棒性。

### 2. 实验设置
- **策略**: 严格执行 "One-for-All" 策略，三个数据集使用完全相同的训练配置，以验证架构本身的强健性。
- **配置**:
  - **模型**: FastViT-RepSS (Small Patch variant)
  - **数据**: 13x13 Patch, 1:9 划分 (10% Training).
  - **策略**: Mixup (0.8), Cutmix (1.0), No EMA, Native AMP, VRAM Cache.
- **数据集差异**:
  - **Indian Pines (IP)**: 16类, 噪声大, 样本极不平衡.
  - **Pavia University (PU)**: 9类, 城市建筑, 边缘清晰.
  - **Salinas (SA)**: 16类, 大面积农田, 纯净度高.

### 3. 实验结果汇总 (SOTA Comparison)

| 数据集 (Dataset) | OA (总体准确率) | AA (平均准确率) | Kappa | 表现评价 |
| :--- | :--- | :--- | :--- | :--- |
| **Salinas (SA)** | **100.00%** | **100.00%** | **1.0000** | **完美收敛**，模型已完全掌握该场景特征。 |
| **Pavia Univ (PU)** | **99.92%** | **99.80%** | **0.9989** | **极优**，在复杂的城市边缘分类中表现惊人。 |
| **Indian Pines (IP)** | **97.68%** | **97.04%** | **0.9735** | **SOTA级**，攻克了高噪声与长尾类别难题。 |

### 4. 训练动态分析 (Dynamics Analysis)
从 WandB 监控曲线中观察到显著的差异化收敛行为：
1.  **Salinas (绿色线)**:
    - **现象**: 起步即巅峰，Loss 极速下降，OA 迅速饱和至 100%。
    - **分析**: 证明模型对纯净的大面积均质区域具有极高的特征提取效率，RepSpectralConv 有效捕捉了清晰的光谱指纹。
2.  **Pavia University (蓝色线)**:
    - **现象**: 训练初期 (Epoch 0-40) 出现剧烈震荡，Loss 波动明显，但后期迅速收敛至 99.9%。
    - **分析**: 这是 Mixup 增强与城市清晰边界 (Sharp Edges) 的对抗过程。模型最终克服了“混合样本”的干扰，学习到了更鲁棒的几何边界特征。
3.  **Indian Pines (紫色线)**:
    - **现象**: 收敛平稳但封顶较早。
    - **分析**: 受限于数据集本身的低空间分辨率和混合像元干扰，97.68% 已接近该数据集在 10% 训练量下的贝叶斯误差下界。

### 5. 项目最终结论
本系列实验成功构建了 **FastViT-RepSS** 高光谱分类框架。
通过 **空谱分离架构设计**、**去步长手术** 以及 **强正则化训练策略**，模型在三个截然不同的基准数据集上均取得了 **Top-tier** 的性能表现。特别是在 Salinas 和 Pavia University 上接近满分的成绩，有力证明了该方法在实际遥感应用中的巨大潜力。